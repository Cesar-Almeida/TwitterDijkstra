...
class Grafo:
    ...
    def dijkstra(self, origem):
        peso = [[-1, 0] for i in range(self.vertices)]
        peso[origem - 1] = [0, origem]
        h = HeapMin()
        ...
        while h.tamanho() > 0:
            ...
            for i in range(self.vertices):
                if self.grafo[v-1][i] != 0:
                    if peso[i][0] == -1 or peso[i][0] > dist + self.grafo[v-1][i]:
                        peso[i] = [dist + self.grafo[v-1][i], v]
